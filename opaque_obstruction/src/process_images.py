...
def adaptive_mask(image, threshold=15, dilate=2):
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    ...
    dilated_mask = dilate_mask(mesh_mask, 2)
    final_mask = cv.erode(dilated_mask, kernel, iterations=1)

    return final_mask

def blurry_mask(image, blur_size=(51, 51), global_thresh=50):
    gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    very_blurred = cv.GaussianBlur(gray, blur_size, 0)
    _, global_mask = cv.threshold(very_blurred, global_thresh, 255, cv.THRESH_BINARY_INV)

    kernel = np.ones((5, 5), np.uint8)
    closed_mask = cv.morphologyEx(global_mask, cv.MORPH_CLOSE, kernel)
...
def run_methods(image, file_name, resize=0.3):    
    ...
    cv.imwrite(rf"..\02_results_inpainted_th\{file_name}_inpainted_ns.png", inpainted_image_ns_th)
    cv.imwrite(rf"..\02_results_inpainted_th\{file_name}_inpainted_fmm.png", inpainted_image_fmm_th)
    
    cv.imwrite(rf"..\02_results_inpainted_ad\{file_name}_inpainted_ns.png", inpainted_image_ns_ad)
    cv.imwrite(rf"..\02_results_inpainted_ad\{file_name}_inpainted_fmm.png", inpainted_image_fmm_ad)
...
if __name__ == "__main__":
    folder_path = rf"..\00_photos_original"
    ...
    for file in files:
        ...
        image_path = rf"..\00_photos_original\{file}"    
        image = cv.imread(image_path)
        run_methods(image, file)